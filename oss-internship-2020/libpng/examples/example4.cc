...
constexpr std::string kOperationFormat = "%d,%d%c";
constexpr std::string kAddFlag = "--add=";
constexpr std::string kSpriteFlag = "--sprite=";
constexpr std::string kSpriteFlagFormat = "%u,%u,%s%c";
...
static int create_sprite(struct Sprite *sprite, int *argc, const char ***argv) {
...
int GetOperationData(const std::string& instr, const int* x, const int* y, const char* z) {
  return std::sscanf(instr.c_str(), kOperationFormat.c_str(), x, y, z);
}

absl::Status AddSprite(sapi::v::Struct<png_image>& image, sapi::v::Array<uint8_t>& buffer,
  ...
  while (index < commands.size() && status.ok()) {
    ...
    if (GetOperationData(commands[index], &x, &y, &tombstone) != 2) {
      status = absl::InternalError(absl::StrCat("--add=", sprite->name, ": invalid position ", commands[index]));
      break;
    }
    ...
    if (x < 0 || y < 0 || static_cast<uint32_t>(x) >= width ||
      ...
      status = absl::InternalError(absl::StrCat("Sprite ", sprite->name, " @ (", x, ",", y, ") outside image"));
      break;
    }
    ...
    ASSERT_THAT(api._rewind_file(&sprite->file), IsOk());
...
absl::Status SimpleoverProcess(sapi::v::Struct<png_image>& image, sapi::v::Array<uint8_t>& buffer, const std::vector<std::string>& commands) {
  bool is_found = true;
  ...
  for (size_t index = 0; index < commands.size() && status.ok();) {
    if (IsSprite(commands[index])) {
      ...
      status = create_sprite(image, buffer, commands, index, sprite);
      ...
      for (auto i = sprites.rbegin(); i != sprites.rend(); ++i) {
        if (i->name == name) {
          successfully = true;
          status = AddSprite(image, buffer, commands, index, i);
          break;
        }
      }
      if (!successfully) {
...
absl::Status LibPNGMain(const std::string infile, const std::vector<std::string>& commands, const std::optional<std::string>& outfile) {
  ...
  *green_background.mutable_data() = {0, 0xff, 0};
  ...
  SAPI_ASSIGN_OR_RETURN(result, api.png_image_finish_read(image.PtrBoth(), green_background.PtrBefore(), buffer.PtrBoth(),
  ...
  auto status = SimpleoverProcess(image, buffer, commands));
  if (!status.ok()) {
    SAPI_RETURN_IF_ERROR(api.png_image_free(image.PtrBoth()));
    return status;
  }
  ...
  if (outfile) {
    ...
    SAPI_ASSIGN_OR_RETURN(result, api.png_image_write_to_file(image.PtrBoth(), outfile_var.PtrBefore(), 0, buffer.PtrBoth(),
      0, sapi::v::NullPtr().PtrBoth()));
    if (!result) {
      status = absl::InternalError(absl::StrCat("simpleover: write to file ", outfile, " failed: ", image.mutable_data()->message));
    }
  } else {
    /// somehow should send "output var"
    SAPI_ASSIGN_OR_RETURN(result, api.png_image_write_to_stdio(image.PtrBoth(), output, 0, buffer.PtrBoth(),
      0, sapi::v::NullPtr().PtrBoth()));
    if (!result) {
      status = absl::InternalError(absl::StrCat("simpleover: write to output failed: ", image.mutable_data()->message));
    }
  }
...
int main(int argc, const char **argv) {
  ...
  auto status = LibPNGMain(commands, out_file_name);
  if (!status.ok()) {
