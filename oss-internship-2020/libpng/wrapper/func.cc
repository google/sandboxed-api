...
void png_read_image_wrapper(png_structrp png_ptr, png_bytep image, size_t height, size_t rowbytes) {
...
void png_write_image_wrapper(png_structrp png_ptr, png_bytep image, size_t height, size_t rowbytes) {
	png_bytep* ptrs = (png_bytep *) malloc(height * sizeof(png_bytep));
	for (size_t i = 0; i != height; ++i) {
		ptrs[i] = image + (i * rowbytes);
	}
