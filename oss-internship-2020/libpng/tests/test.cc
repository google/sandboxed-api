...
TEST(SandboxTest, ReadWrite) {
  ...
  image.mutable_data()->version = PNG_IMAGE_VERSION;
	...
	auto status_or_int = api.png_image_begin_read_from_file(image.PtrBoth(), infile_var.PtrBefore());
	ASSERT_THAT(status_or_int, IsOk()) << "png_image_begin_read_from_file fatal error";
	ASSERT_THAT(status_or_int.value(), IsTrue()) << "png_image_begin_read_from_file failed: " << image.mutable_data()->message;

  image.mutable_data()->format = PNG_FORMAT_RGBA;
	ASSERT_THAT(image.mutable_data()->version, Eq(PNG_IMAGE_VERSION)) << "image version changed";
	...
	ASSERT_THAT(status_or_int.value(), IsTrue()) << "png_image_finish_read failed: " << image.mutable_data()->message;
	ASSERT_THAT(image.mutable_data()->version, Eq(PNG_IMAGE_VERSION)) << "image version changed";
	ASSERT_THAT(image.mutable_data()->format, Eq(PNG_FORMAT_RGBA)) << "image format changed";
