...
absl::StatusOr<std::vector<MapsEntry>> ParseProcMaps(
  ...
  static constexpr char kFormatString[] = "%lx-%lx %c%c%c%c %lx %x:%x %lu %zn";
  ...
  for (const auto& line : lines) {
    ...
    char r, w, x, s;
    size_t nbytes;
    int n_matches = sscanf(line.c_str(), kFormatString, &entry.start,
                           &entry.end, &r, &w, &x, &s, &entry.pgoff,
                           &entry.major, &entry.minor, &entry.inode, &nbytes);
    if (n_matches != 10) {
      return absl::FailedPreconditionError("Invalid format");
    }
    entry.path = line.substr(nbytes);
