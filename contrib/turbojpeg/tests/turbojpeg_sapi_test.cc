...
// clang-format off
const unsigned char image[2412] = {
...
unsigned int image_len = 2412;
// clang-format on

#ifdef DEBUG_LOGGING
#define MADE_IT                                                   \
  do {                                                            \
    std::cerr << "Reached " << __FILE__ << ':' << __LINE__ << ' ' \
...
TEST_F(TurboJpegSapiSandboxTest, Compressor) {
  static_assert(
      std::is_same_v<decltype(api_->tjInitCompress()), absl::StatusOr<void*>>,
      "bug");
  ...
  unsigned long actual_length;
  {
    ...
    auto result = api_->tjCompress2(
        &compression_handle, array.PtrBefore(), 12, 36, 67, TJPF_RGB,
        buffer.PtrAfter(), length.PtrBoth(), TJSAMP_444, 10,
        0 * (TJFLAG_ACCURATEDCT | TJFLAG_PROGRESSIVE | TJFLAG_LIMITSCANS));
  ...
  static_assert(std::is_same_v<decltype(value), uintptr_t>, "bug");

  MADE_IT;

  static_assert(
      std::is_same_v<decltype(api_->tjInitDecompress()), absl::StatusOr<void*>>,
      "bug");
  ...
  sapi::v::Int width{0}, height{0}, subsamp{0}, colorspace{0};
  ...
  static_assert(sizeof image == 12 * 67 * 3, "bug");
