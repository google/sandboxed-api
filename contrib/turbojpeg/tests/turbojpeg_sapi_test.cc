...
TEST_F(TurboJpegSapiSandboxTest, Decompressor) {
  ...
  auto decompress_result = api_->tjDecompressHeader3(
      &decompression_handle, array.PtrBefore(), result->size(), width.PtrAfter(),
  ...
  if (decompress_result.value() != 0) {
    auto errmsg = api_->tjGetErrorStr2(decompression_handle.PtrNone());
    ASSERT_THAT(errmsg, IsOk());
  ...
  decompress_result =
      api_->tjDecompress2(&decompression_handle, array.PtrBefore(), result->size(),
