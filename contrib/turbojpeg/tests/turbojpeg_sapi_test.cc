...
class TurboJpegSapiSandboxTest : public testing::Test {
 protected:
  ...
  static absl::StatusOr<std::string> GetTurboJpegErrorStr(sapi::v::Ptr* decompression_handle) {
    auto errmsg = api_->tjGetErrorStr2(decompression_handle);
    SAPI_RETURN_IF_ERROR(errmsg);
    return sandbox_->GetCString(sapi::v::RemotePtr(errmsg.value()), 256);
  }
...
TEST_F(TurboJpegSapiSandboxTest, Decompressor) {
  ...
  ASSERT_THAT(decompress_result.value(), Eq(0))
      ...
      << GetTurboJpegErrorStr(&decompression_handle).value();
