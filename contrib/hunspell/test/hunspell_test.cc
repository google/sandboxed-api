...
class HunspellTest : public ::testing::Test {
 protected:
  static constexpr char encoding_[] = "UTF-8";
  static constexpr char affix_file_name_[] = "utf8.aff";
  static constexpr char dictionary_file_name_[] = "utf8.dic";

  static constexpr char good_file_name_[] = "utf8.good";
  static constexpr char wrong_file_name_[] = "utf8.wrong";

  static constexpr char suggestion_[] = "fo";
  static constexpr char random_word_[] = "random_word123";

  void SetUp() override {
    test_files_dir_ = getenv("TEST_FILES_DIR");
    ASSERT_NE(test_files_dir_, nullptr);

    std::string s_afn = GetTestFilePath(affix_file_name_);
    std::string s_dfn = GetTestFilePath(dictionary_file_name_);
...
TEST_F(HunspellTest, CheckEncoding) {
  ...
  EXPECT_EQ(encoding, encoding_);
}

TEST_F(HunspellTest, CheckGoodSpell) {
  SAPI_ASSERT_OK_AND_ASSIGN(char* ret,
                            api_->Hunspell_get_dic_encoding(&(*hunspellrp_)));
  std::ifstream wtclst(GetTestFilePath(good_file_name_), std::ios_base::in);
  ...
  while (std::getline(wtclst, buf)) {
    ...
    SAPI_ASSERT_OK_AND_ASSIGN(
        int rdp, api_->Hunspell_spell(&(*hunspellrp_), cbuf.PtrBefore()));
    ASSERT_EQ(rdp, 1);
  }
}
...
TEST_F(HunspellTest, CheckWrongSpell) {
  SAPI_ASSERT_OK_AND_ASSIGN(char* ret,
                            api_->Hunspell_get_dic_encoding(&(*hunspellrp_)));
  std::ifstream wtclst(GetTestFilePath(wrong_file_name_), std::ios_base::in);
  ASSERT_TRUE(wtclst.is_open());
...
TEST_F(HunspellTest, CheckAddToDict) {
  sapi::v::ConstCStr cbuf(random_word_);

  int rdp;
  SAPI_ASSERT_OK_AND_ASSIGN(
      rdp, api_->Hunspell_spell(&(*hunspellrp_), cbuf.PtrBefore()));
  ASSERT_EQ(rdp, 0);

  SAPI_ASSERT_OK_AND_ASSIGN(
      rdp, api_->Hunspell_add(&(*hunspellrp_), cbuf.PtrBefore()));
  ASSERT_EQ(rdp, 0);
  ...
  SAPI_ASSERT_OK_AND_ASSIGN(
      rdp, api_->Hunspell_remove(&(*hunspellrp_), cbuf.PtrBefore()));
  ASSERT_EQ(rdp, 0);
...
TEST_F(HunspellTest, CheckSuggestion) {
  sapi::v::ConstCStr cbuf(suggestion_);
