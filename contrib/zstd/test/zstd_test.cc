...
TEST(SandboxTest, CheckCompressAndDecompressStream) {
  ...
  ASSERT_THAT(status, IsOk()) << "Unable to decompress decompress";
...
TEST(SandboxTest, CheckCompressAndDecompressStreamFD) {
  ...
  ASSERT_GE(outmiddlepos, 0);

  ASSERT_LT(outmiddlepos, inpos);
